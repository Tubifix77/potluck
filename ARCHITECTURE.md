...
**The test fleet.** The owner supplies example systems as architecture tests, under a standing rule that is now doctrine: *if a named setup is not possible, the architecture is wrong.* Five are on file beyond the car and the home:
...
| **Production-line test fixtures** | many near-identical nodes; per-unit traceability; human hands inside the work envelope | Possible, and the first named system that **fails a v1 scope boundary rather than sitting inside one**: a floor of forty fixtures exceeds §4's single ESP-NOW cell of ~20 nodes, so this is a wired deployment (§5.3.1) or it waits for the routed post-v1 profile (§5.3). That is the scope statement earning its keep on a real example instead of in a table cell. Everything else lands cleanly: stimulus and sampling inside a fixture are L0/L1 and pin themselves there, the pass/fail verdict and line-level aggregation are L3/L4, per-unit traceability is §7.6's capture replayed against its digest with the unit's serial carried as an ordinary namespace entry, and a test-program update is §7.4's signed package with trial-commit revert across every fixture at once. Two non-claims, stated because a factory will assume otherwise: calibration certificates and audit-grade traceability are a metrology apparatus this document does not provide, and any lid, press or clamp in the work envelope is §12 — Potluck instruments the fixture and never holds the interlock. |
